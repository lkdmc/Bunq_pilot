...
export default function FinancialReport({ onBack }) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${API_BASE}/api/receipts/report`)
      .then(res => res.json())
  ...
  const BackButton = () => (
    <button onClick={onBack} className="flex items-center gap-1.5 text-gray-400 hover:text-white transition-colors mb-5 -ml-1">
  ...
  if (loading) {
    return (
      <div className="flex-1 overflow-y-auto pb-[90px] bg-black text-white px-4 pt-12 flex flex-col items-center justify-center">
  ...
  if (!data || !data.breakdown || data.breakdown.length === 0) {
    return (
      <div className="flex-1 overflow-y-auto pb-[90px] bg-black text-white px-4 pt-12">
        <BackButton />
        <h1 className="text-3xl font-bold mb-6">Receipt Report</h1>
        <div className="bg-[#1C1C1E] border border-[#2C2C2E] rounded-2xl p-6 text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">Tap the 📷 button on any transaction to scan a receipt.</p>
        </div>
      </div>
    );
  }
  ...
  data.breakdown.forEach(item => {
    ...
    if (subcatTotals[sub] !== undefined) subcatTotals[sub] += item.total;
    else subcatTotals[sub] = item.total;
    if (!catTotals[cat]) catTotals[cat] = { name: cat, Essential: 0, Standard: 0, Luxury: 0, total: 0 };
    if (catTotals[cat][sub] !== undefined) catTotals[cat][sub] += item.total;
  ...
  const barData = Object.values(catTotals).sort((a, b) => b.total - a.total).slice(0, 5);
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-[90px] bg-black text-white px-4 pt-12">
      <BackButton />
      <h1 className="text-3xl font-bold mb-1">Receipt Report</h1>
      ...
      <div className="bg-gradient-to-br from-[#1C1C1E] to-black border border-[#2C2C2E] rounded-2xl p-5 mb-6 relative overflow-hidden">
        ...
        <p className="text-gray-300 text-sm leading-relaxed">{data.advice}</p>
      </div>

      {/* Spending Behavior Pie */}
